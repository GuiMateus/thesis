...
class segmentationInit():
    ...
    def deeplabInit(self, reconstructionType):
        ...
        if reconstructionType == "online":
            ...
            print(model)
            weights = torch.load(
                '/opt/vision/weights/deeplab/model_dynamic.pth.tar', map_location='cpu')['state_dict']
            ...
            print(model)
            weights = torch.load(
                '/opt/vision/weights/deeplab/model_static.pth.tar', map_location='cpu')['state_dict']
    ...
    def inference(self, model, tensorArray, reconstructionType, image):
        ...
        for tensor in tensorArray:
            with torch.no_grad():
                ...
                if reconstructionType == "online":
                    maskOutput = make_grid(decode_seg_map_sequence(torch.max(output[:3], 1)[
                                           1].detach().cpu().numpy(), dataset="online"), 3, normalize=False, range=(0, 255))
                elif reconstructionType == "offline":
                    maskOutput = make_grid(decode_seg_map_sequence(torch.max(output[:3], 1)[1].detach(
                    ).cpu().numpy(), dataset="offline"), 3, normalize=False, range=(0, 255))
    ...
    def toImgCoord(self, masks, depth, feedback, reconstructionType):
        ...
        if len(masks) is not 0:
            ...
            for detection in self.crops:
                ...
                currentMask = cv2.resize(
                    currentMask, (int(xmax-xmin), int(ymax-ymin)))
                ...
                if cv2.countNonZero(maskGrey) is not 0:
                    ...
                    _, mask = cv2.threshold(
                        maskGrey, 10, 255, cv2.THRESH_BINARY)
                    ...
                    if roi.shape[0] == maskInv.shape[0] and roi.shape[1] == maskInv.shape[1]:
                        if roi is not None and maskInv is not None:
                            ...
                            extractMask = cv2.bitwise_and(
                                currentMask, currentMask, mask=mask)

                            maskArray.append(extractMask)
                            dst = cv2.add(imgROI, extractMask)
                            # print(detection[1])
                            feedback[ymin:ymax, xmin:xmax] = dst
                            objectId = detection[1] + "_uid_" + \
                                str(random.randint(0, 255))
                            cv2.putText(
                                feedback, objectId, (cX, cY), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
                            cv2.circle(feedback, (cX, cY), 5,
                                       (255, 255, 255), -1)
            ...
            if reconstructionType == "offline" and jsonObject != {} and self.crops != None:
                ...
                if croppedImage is not None and croppedImage.shape[0] > 10 and croppedImage.shape[1] > 10:
                    if reconstructionType == "online":
                        croppedImage = cv2.resize(
                            croppedImage, self.deepLabDimensionsOnline)
                    elif reconstructionType == "offline":
                        croppedImage = cv2.resize(
                            croppedImage, self.deepLabDimensionsOffline)

                    self.crops.append(
                        [croppedImage, label, confidence, self.newBbox, inputImage.shape[0], inputImage.shape[1]])
                    label = str(label)
    ...
    def imageToTensor(self):
        ...
        # for croppedIndex in self.crops:
            ...
            tensorArray.append([tensor, croppedIndex[1], croppedIndex[2],
                               croppedIndex[3], croppedIndex[4], croppedIndex[5]])
        return tensorArray
